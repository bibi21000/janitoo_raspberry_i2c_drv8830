...
class PwmComponent(JNTComponent):
    ...
    def set_level(self, node_uuid, index, data):
        ...
        self._bus.i2c_acquire()
        try:
            self._bus.drv8830_manager.setPWM(p, int(data*4096/100),0)
    ...
    def set_switch(self, node_uuid, index, data):
        ...
        if data == "on":
            ...
            try:
                p = self.values['num'].get_data_index(index=index)
                self._bus.drv8830_manager.setPWM(p, 4096, 0)
            ...
            try:
                p = self.values['num'].get_data_index(index=index)
                self._bus.drv8830_manager.setPWM(p, 0, 4096)
    ...
    def set_change(self, node_uuid, index, data):
        ...
        try:
            ...
            if data=="-1,-1":
                ...
                sx,sy = data.split(',')
            self._bus.drv8830_manager.setPWM(px, int(sx), 4096-int(sx))
            self._bus.drv8830_manager.setPWM(py, int(sy), 4096-int(sy))
        except Exception:
